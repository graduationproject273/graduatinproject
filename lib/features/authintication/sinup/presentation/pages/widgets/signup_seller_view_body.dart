...
class _SellerStepsPageState extends State<SellerStepsPage> {
          ...
          listener: (context, state) {
            if (state is SellerSuccess) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text("Seller registration successful!"),
  ...
  Widget _buildStepContent(int step) {
    ...
    switch (step) {
      ...
      case 1:
        return PickupAddressWidget();
      case 2:
        return DetailsInSellerSignupWidget();
      case 3:
        return SupplierDetailsInSellerSignup();
      default:
  ...
  bool _validateCurrentStep(SellerCubit cubit, int step) {
    switch (step) {
      case 0:
        if (cubit.TINController.text.isEmpty ||
            cubit.mobileController.text.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
      ...
      case 3:
        if (cubit.bussinessNameController.text.isEmpty ||
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Please fill all supplier details")),
          );
          return false;
        }
        return true;
